...
tetha = math.acos(c/(VP*refindex1)) # Ángulo de emisión
...
def moveParticle(initx, posx, posy, color, r, surface, background_col, trace_width, trace_col, rads, vl):
	...
	for i in range(n):
		if posx >= rads[i][0]:
			pygame.draw.circle(window,trace_col,(rads[i][0],posy),int(v*rads[i][1])+1,trace_width)
...
def main():
	...
	while True:
		...
		moveParticle(initx,int(x),Y,RED,3,window,WHITE,1,(22,44,66),rads,v)
		x += 0.13
